...
class AutoMinecartCommandExecutor implements CommandExecutor {
    public static final String COMMAND_NS = "autominecart";

    private final AutoMinecart plugin;

    AutoMinecartCommandExecutor(AutoMinecart plugin) {
        this.plugin = plugin;
    }
        ...
        if (command.getName().equalsIgnoreCase(COMMAND_NS)) {
            ...
            if((args.length >= 1) && (args[0].equalsIgnoreCase("reload"))) {
                if (sender.hasPermission(COMMAND_NS + ".reload")) {
                    ...
                    customLogger.info("AutoMinecart config reloaded.");
                    return true;
                }
            }
            else if((args.length == 0) || (args[0].equalsIgnoreCase("help"))) {
                String helpString = "==== AutoMinecart help ====\n";
